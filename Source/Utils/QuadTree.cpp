...
namespace QuadTree {
	QuadTreeEntry::QuadTreeEntry(GameObjectEntity* go) {
		gameObject = go;
	...
	Node::Node(Physics::AABB aabb, size_t entries) {
		bounds = aabb;
		max_entries = entries;
	}
	...
	std::vector<Node*> Node::GetValidCollisionNodes(Physics::AABB aabb) {
		...
		if (!isLeaf) {
			for (Node* node : subdivisions) {
				for (Node* n2 : node->GetValidCollisionNodes(aabb)) {
					result.push_back(n2);
				}
			}
		}
	...
	std::vector<GameObjectEntity*> Node::GetPotentialCollisionTargets(GameObjectEntity* gameObject, std::vector<GameObjectEntity*> ignoredObjects, GameObjectEntity::PhysicsType type_filter) {
		Physics::AABB aabb{};
		aabb.min.x = gameObject->position.x - gameObject->scale.x * 0.5f;
		aabb.min.y = gameObject->position.y - gameObject->scale.y * 0.5f;
		aabb.max.x = gameObject->position.x + gameObject->scale.x * 0.5f;
		aabb.max.y = gameObject->position.y + gameObject->scale.y * 0.5f;
		std::vector<Node*> validNodes = GetValidCollisionNodes(aabb);

		std::vector<GameObjectEntity*> result;

		for (Node* node : validNodes) {
			for (QuadTreeEntry* entry : node->entries) {
				if (!entry->gameObject->is_active) 
					continue;
